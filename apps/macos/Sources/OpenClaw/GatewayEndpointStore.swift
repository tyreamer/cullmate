...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.baxbot", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.baxbot", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            // Cullmate defaults to local mode — gateway should always auto-start.
            initialMode = .local
        }
