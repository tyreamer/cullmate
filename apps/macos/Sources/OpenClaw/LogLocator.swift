...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/baxbot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("baxbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("baxbot-gateway.log")
    }
    ...
    /// Returns the newest log file under the log directory (rolling or stdout), or nil if none exist.
